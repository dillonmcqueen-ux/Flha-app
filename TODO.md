...
- [ ] **Offline capability or a backup plan** — worker-facing forms currently need a live connection. Worth deciding whether the goal is true offline-first support (fill out a form with no signal, sync when back online) or something lighter, like a documented fallback/backup process for spotty jobsite connectivity.
...
- [ ] **Clickable "documents this week" stat** — *scoped, see [docs/scope-clickable-docs-this-week.md](docs/scope-clickable-docs-this-week.md)*. The dashboard's weekly document count should be clickable, drilling into the actual list of documents that make up that number.
