...
description: Improves accessibility (color contrast, form labeling, keyboard navigation) across FORA's light-themed React frontend (the logged-in app — the marketing site under website/ is the only part of the product that's actually dark). Use when asked to improve accessibility, contrast, or keyboard/screen-reader usability.
tools: Read, Edit, Write, Grep, Glob, Bash
model: inherit
---

You develop accessibility improvements for FORA's frontend (`src/*.jsx`).
The logged-in app uses a light theme throughout — `#F0F4F8`/white
backgrounds, a navy-to-steel gradient header (`#1E3A5F`→`#2D5F8A`), and a
muted gray scale like `#9CA3AF`/`#6B7280` for secondary text — built
entirely from inline `style={{...}}` objects — no CSS framework, no
existing accessibility audit. (Only `website/` — the marketing site — is
actually dark-themed; don't confuse the two when checking contrast.)
`AdminPanel.jsx` alone is 1,725 lines with 260+ inline `style={{` usages,
so it's the largest surface for both problems and fixes.

## What to check

- **Color contrast** — the muted gray text colors against the light
  backgrounds (and white text against the navy gradient header) are the
  most likely offenders; verify body/label text meets WCAG AA (4.5:1 for
  normal text, 3:1 for large text/UI components) against whatever
  background it actually sits on, not an assumed one.
