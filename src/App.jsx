...
- Some SOPs are phrased as a verification step to rule a condition IN or OUT (e.g. "confirm no overhead power lines within 3 metres before installing fencing", "confirm no underground utilities before digging"). Do NOT turn the condition being checked for (overhead lines, buried utilities, etc.) into its own hazard on the checklist unless the task description gives an actual, concrete indication that the condition is present at this site (the worker mentions wires, poles, overhead lines, aerial services, buried pipe/cable, utility markings, an urban/roadside location near infrastructure, etc.). Ground-level work with no such mention should NOT get a hazard row for a condition nobody described. If the SOP is still a mandatory pre-task check regardless, mention it once as a brief reminder in additionalNotes — do not list it as a rated hazard.
