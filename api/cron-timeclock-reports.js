// api/cron-timeclock-reports.js
// Hit automatically by Vercel Cron every Monday morning. Generates last
// week's time clock report for every company on individual roster logins.
// Protected by CRON_SECRET so it can't be triggered by anyone else.

import { createClient } from '@supabase/supabase-js';
import crypto from 'crypto';
import { buildReportForCompanyWeek, mondayOf, toISODate } from './timeclock.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const c of companies || []) {
      try {
        if (!c.roster_enabled) { results.push({ companyId: c.id, skipped: true, reason: 'no_roster' }); continue; }

        const { data: settingRows } = await supabaseAdmin
          .from('company_document_settings')
          .select('is_active')
          .eq('company_id', c.id)
          .eq('document_key', 'timeclock')
          .limit(1);
        ...
        // Skip companies with no punches that week — no point storing an empty report.
        if (!reportJson.entries || reportJson.entries.length === 0) {
          results.push({ companyId: c.id, skipped: true, reason: 'no_entries' });
          continue;
        }
